...
if args.variant == 'vanilla':
    model = model.GPT(mconf)   #在默认情况下addctive=false
    model = model.to(device)
    #pass
    # TODO [part c]: Make some model here
elif args.variant == 'synthesizer':
    #mconf.additsive = True
...
if args.function == 'pretrain':
    ...
    # fine_tune_dataset_dev = dataset.NameDataset(pretrain_dataset,open('birth_dev.tsv',encoding='utf-8').read())
    train_config = trainer.TrainerConfig(max_epochs=training_max_epchos, batch_size=256, learning_rate=6e-4,
                  lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                  num_workers=0)
    Trainer = trainer.Trainer(model = model,train_dataset=fine_tune_dataset_train,test_dataset = None,config = train_config)
    Trainer.train()
