...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # print(enc_hiddens)
        enc_hiddens = torch.Tensor.permute(enc_hiddens,(1,0,2))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_ = torch.split(Y,split_size_or_sections=1)
        for Y_t in Y_:
            Y_t_squeezed = torch.squeeze(Y_t,dim = 0)
            Y_bar_t = torch.cat((Y_t_squeezed,o_prev),dim = 1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t  = torch.squeeze(torch.bmm(enc_hiddens_proj,torch.unsqueeze(dec_hidden,dim=2)),dim = 2) #相对于每个src token的注意力
